...
namespace zero_cost_serialization::map {
	...
	enum class link : std::size_t {};

	[[maybe_unused]] inline constexpr auto nil = map::link{};

	namespace detail {
		constexpr auto make_link(std::unsigned_integral auto x)
		{
			return static_cast<map::link>(static_cast<std::size_t>(x) + 1);
		}

		template <typename R>
		constexpr auto index(map::link x)
		{
		...
		concept read_proj = requires(R&& rng, Proj proj, map::link r, T t)
		{
			{ t = std::invoke(proj, std::forward<R>(rng)[detail::index<R>(r)]) };
			requires std::copy_constructible<Proj>;
		};

		template <typename Proj, typename R, typename T>
		concept write_proj = requires(R&& rng, Proj proj, map::link r, T t)
		{
		...
		concept write_color_proj = write_proj<Proj, R, map::color>;

		template <typename Proj, typename R>
		concept write_link_proj = write_proj<Proj, R, map::link>;

		template <typename Proj, typename R>
		concept read_color_proj = read_proj<Proj, R, map::color>;

		template <typename Proj, typename R>
		concept read_link_proj = read_proj<Proj, R, map::link>;

		template <typename R, typename... P, std::size_t... Is>
		constexpr decltype(auto) ref_helper(R&& rng, map::link x, std::index_sequence<Is...>, P... proj)
		{
			auto r = x;
		...
		constexpr decltype(auto) ref(R&& rng, map::link x, P... proj)
		{
		...
		constexpr auto val_helper(R&& rng, map::link x, std::index_sequence<Is...>, P... proj) -> T
		{
			auto r = x;
			auto last = (..., [&] { if constexpr (Is not_eq sizeof...(P) - 1) r = std::invoke(proj, std::forward<R>(rng)[detail::index<R>(r)]); else return proj; }());
			if constexpr (std::same_as<T, map::color>) {
				if (map::nil == r)
					return map::color::black;
				return std::invoke(last, std::forward<R>(rng)[detail::index<R>(r)]);
			} else {
				static_assert(std::same_as<T, map::link>);
				return std::invoke(last, std::forward<R>(rng)[detail::index<R>(r)]);
			}
		}

		template <typename T, typename R, typename... P>
		constexpr auto val(R&& rng, map::link x, P... proj)
		{
		...
		{
			const auto y = detail::val<map::link>(std::forward<R>(rng), x, right);
			detail::ref(std::forward<R>(rng), x, right) = detail::val<map::link>(std::forward<R>(rng), y, left);
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), y, left))
				detail::ref(std::forward<R>(rng), y, left, parent) = x;
			detail::ref(std::forward<R>(rng), y, parent) = detail::val<map::link>(std::forward<R>(rng), x, parent);
			if (map::nil == detail::val<map::link>(std::forward<R>(rng), x, parent))
				root = y;
			else if (x == detail::val<map::link>(std::forward<R>(rng), x, parent, left))
		...
		{
			const auto y = detail::val<map::link>(std::forward<R>(rng), z, parent, parent, right);
			if (map::color::red == detail::val<map::color>(std::forward<R>(rng), y, color)) {
				detail::ref(std::forward<R>(rng), z, parent, color) = map::color::black;
				detail::ref(std::forward<R>(rng), y, color) = map::color::black;
				detail::ref(std::forward<R>(rng), z, parent, parent, color) = map::color::red;
				z = detail::val<map::link>(std::forward<R>(rng), z, parent, parent);
			} else {
				if (z == detail::val<map::link>(std::forward<R>(rng), z, parent, right)) {
					z = detail::val<map::link>(std::forward<R>(rng), z, parent);
					root = detail::left_rotate(std::forward<R>(rng), root, z, left, right, parent);
				}
				detail::ref(std::forward<R>(rng), z, parent, color) = map::color::black;
				detail::ref(std::forward<R>(rng), z, parent, parent, color) = map::color::red;
				root = detail::left_rotate(std::forward<R>(rng), root, detail::val<map::link>(std::forward<R>(rng), z, parent, parent), right, left, parent);
			}
		...
		{
			while (map::color::red == detail::val<map::color>(std::forward<R>(rng), z, parent, color))
				if (detail::val<map::link>(std::forward<R>(rng), z, parent) == detail::val<map::link>(std::forward<R>(rng), z, parent, parent, left))
			...
			detail::ref(std::forward<R>(rng), root, color) = map::color::black;
			return root;
		}
		...
		{
			while (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), x, right))
				x = detail::val<map::link>(std::forward<R>(rng), x, right);
			return x;
		}
		...
		{
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), x, right))
				return detail::max(std::forward<R>(rng), detail::val<map::link>(std::forward<R>(rng), x, right), left);
			auto y = detail::val<map::link>(std::forward<R>(rng), x, parent);
			while (map::nil not_eq y and x == detail::val<map::link>(std::forward<R>(rng), y, right)) {
				x = y;
				y = detail::val<map::link>(std::forward<R>(rng), y, parent);
			}
			return y;
		}
		...
		{
			auto w = detail::val<map::link>(std::forward<R>(rng), xp, right);
			if (map::color::red == detail::val<map::color>(std::forward<R>(rng), w, color)) {
				detail::ref(std::forward<R>(rng), w, color) = map::color::black;
				detail::ref(std::forward<R>(rng), xp, color) = map::color::red;
				root = detail::left_rotate(std::forward<R>(rng), root, xp, left, right, parent);
				w = detail::val<map::link>(std::forward<R>(rng), xp, right);
			}
			if (map::color::black == detail::val<map::color>(std::forward<R>(rng), w, left, color) and map::color::black == detail::val<map::color>(std::forward<R>(rng), w, right, color)) {
				detail::ref(std::forward<R>(rng), w, color) = map::color::red;
				x = xp;
				xp = detail::val<map::link>(std::forward<R>(rng), x, parent);
			} else {
				if (map::color::black == detail::val<map::color>(std::forward<R>(rng), w, right, color)) {
					detail::ref(std::forward<R>(rng), w, left, color) = map::color::black;
					detail::ref(std::forward<R>(rng), w, color) = map::color::red;
					root = detail::left_rotate(std::forward<R>(rng), root, w, right, left, parent);
					w = detail::val<map::link>(std::forward<R>(rng), xp, right);
				}
				detail::ref(std::forward<R>(rng), w, color) = detail::val<map::color>(std::forward<R>(rng), xp, color);
				detail::ref(std::forward<R>(rng), xp, color) = map::color::black;
				if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), w, right))
					detail::ref(std::forward<R>(rng), w, right, color) = map::color::black;
		...
		{
			while (root not_eq x and map::color::black == detail::val<map::color>(std::forward<R>(rng), x, color))
				if (map::nil not_eq xp and detail::val<map::link>(std::forward<R>(rng), xp, left) == x)
			...
			if (map::nil not_eq x)
				detail::ref(std::forward<R>(rng), x, color) = map::color::black;
			return root;
		}
		...
		{
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), z, parent)) {
				if (detail::val<map::link>(std::forward<R>(rng), z, parent, left) == z)
					detail::ref(std::forward<R>(rng), z, parent, left) = y;
				else
					detail::ref(std::forward<R>(rng), z, parent, right) = y;
			}
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), z, left))
				detail::ref(std::forward<R>(rng), z, left, parent) = y;
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), z, right))
				detail::ref(std::forward<R>(rng), z, right, parent) = y;
			if (root == z)
				root = y;
			detail::ref(std::forward<R>(rng), y, color) = detail::val<map::color>(std::forward<R>(rng), z, color);
			detail::ref(std::forward<R>(rng), y, left) = detail::val<map::link>(std::forward<R>(rng), z, left);
			detail::ref(std::forward<R>(rng), y, right) = detail::val<map::link>(std::forward<R>(rng), z, right);
			detail::ref(std::forward<R>(rng), y, parent) = detail::val<map::link>(std::forward<R>(rng), z, parent);
			return root;
		}
		...
		constexpr auto insert(R&& rng, map::link root, map::link z, Left left, Right right, Parent parent, Color color, Less less = {}, Key key = {})
		{
			auto y = map::nil;
			auto x = root;
			while (map::nil not_eq x) {
				y = x;
				if (std::invoke(less, detail::ref(std::forward<R>(rng), z, key), detail::ref(std::forward<R>(rng), x, key)))
					x = detail::val<map::link>(std::forward<R>(rng), x, left);
				else
					x = detail::val<map::link>(std::forward<R>(rng), x, right);
			}
			detail::ref(std::forward<R>(rng), z, color) = map::color::red;
			detail::ref(std::forward<R>(rng), z, left) = map::nil;
			detail::ref(std::forward<R>(rng), z, right) = map::nil;
		...
		{
			auto y = z;
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), z, left) and map::nil not_eq detail::val<map::link>(std::forward<R>(rng), z, right))
				y = detail::successor(std::forward<R>(rng), z, left, right, parent);
			auto y_child = map::nil not_eq detail::val<map::link>(std::forward<R>(rng), y, left) ? detail::val<map::link>(std::forward<R>(rng), y, left) : detail::val<map::link>(std::forward<R>(rng), y, right);
			if (map::nil == detail::val<map::link>(std::forward<R>(rng), y, parent))
				root = y_child;
			else if (y == detail::val<map::link>(std::forward<R>(rng), y, parent, left))
				detail::ref(std::forward<R>(rng), y, parent, left) = y_child;
			else
				detail::ref(std::forward<R>(rng), y, parent, right) = y_child;
			if (map::nil not_eq y_child)
				detail::ref(std::forward<R>(rng), y_child, parent) = detail::val<map::link>(std::forward<R>(rng), y, parent);
			auto y_color = detail::val<map::color>(std::forward<R>(rng), y, color);
			auto y_child_parent = detail::val<map::link>(std::forward<R>(rng), y, parent) == z ? y : detail::val<map::link>(std::forward<R>(rng), y, parent);
			if (y not_eq z)
		...
		{
			return map::nil not_eq curr and static_cast<std::size_t>(curr) <= rng.size();
		}

		template <typename R, typename Left, typename Right, typename Parent>
		constexpr auto validate_link(R&& rng, map::link curr, map::link next, Left left, Right right, Parent parent)
		{
			return detail::val<map::link>(std::forward<R>(rng), curr, parent) == next and (map::nil == detail::val<map::link>(std::forward<R>(rng), curr, left) or map::nil == detail::val<map::link>(std::forward<R>(rng), curr, right) or detail::val<map::link>(std::forward<R>(rng), curr, left) not_eq detail::val<map::link>(std::forward<R>(rng), curr, right));
		}
		...
		{
			for (; map::nil not_eq detail::val<map::link>(std::forward<R>(rng), curr, left); curr = detail::val<map::link>(std::forward<R>(rng), curr, left)) {
				if (not detail::validate_index(std::forward<R>(rng), detail::val<map::link>(std::forward<R>(rng), curr, left)))
					return std::nullopt;
				if (not detail::validate_link(std::forward<R>(rng), detail::val<map::link>(std::forward<R>(rng), curr, left), curr, left, right, parent))
					return std::nullopt;
				if (detail::val<map::color>(std::forward<R>(rng), curr, left, color) == map::color::black)
					++black_height;
			}
		...
		{
			if (map::nil not_eq detail::val<map::link>(std::forward<R>(rng), curr, right)) {
				if (not detail::validate_index(std::forward<R>(rng), detail::val<map::link>(std::forward<R>(rng), curr, right)))
					return std::nullopt;
				if (not detail::validate_link(std::forward<R>(rng), detail::val<map::link>(std::forward<R>(rng), curr, right), curr, left, right, parent))
					return std::nullopt;
				if (detail::val<map::color>(std::forward<R>(rng), curr, right, color) == map::color::black)
					++black_height;
				if (auto min = detail::validate_min(std::forward<R>(rng), detail::val<map::link>(std::forward<R>(rng), curr, right), black_height, left, right, parent, color))
					std::tie(curr, black_height) = min.value();
				else
					return std::nullopt;
			} else
				for (auto done = false; not done; done = map::nil == detail::val<map::link>(std::forward<R>(rng), curr, parent) or detail::val<map::link>(std::forward<R>(rng), curr, parent, right) not_eq curr, curr = detail::val<map::link>(std::forward<R>(rng), curr, parent))
					if (detail::val<map::color>(std::forward<R>(rng), curr, color) == map::color::black)
						--black_height;
			return std::make_optional(std::pair{curr, black_height});
		}
		...
		struct min_fn
		{
			...
			constexpr auto operator()(R&& rng, map::link root, Left left) const
			{
				return map::nil not_eq root ? detail::max(std::forward<R>(rng), root, left) : root;
			}
		};
		...
		struct max_fn
		{
			...
			constexpr auto operator()(R&& rng, map::link root, Right right) const
			{
				return map::nil not_eq root ? detail::max(std::forward<R>(rng), root, right) : root;
			}
		};
		...
		struct insert_fn
		{
			...
			constexpr auto operator()(R&& rng, map::link root, std::ranges::range_size_t<R> index, Left left, Right right, Parent parent, Color color, Less less = {}, Key key = {}) const
			{
		...
		struct erase_fn
		{
			...
			constexpr auto operator()(R&& rng, map::link root, std::ranges::range_size_t<R> index, Left left, Right right, Parent parent, Color color) const
			{
		...
		struct validate_fn
		{
			...
			requires (std::indirect_strict_weak_order<Less, std::projected<std::ranges::iterator_t<R>, Key>> and std::copy_constructible<Less> and std::copy_constructible<Key>)
			constexpr auto operator()(R&& rng, map::link root, Left left, Right right, Parent parent, Color color, Less less = {}, Key key = {}) const
			{
				...
				if (not detail::validate_index(std::forward<R>(rng), root))
					return false;
				if (not detail::validate_link(std::forward<R>(rng), root, map::nil, left, right, parent))
					return false;
				if (detail::val<map::color>(std::forward<R>(rng), root, color) not_eq map::color::black)
					return false;
				...
				for (auto prev = root; map::nil not_eq prev; prev = root) {
					if ((map::nil == detail::val<map::link>(std::forward<R>(rng), root, left) or map::nil == detail::val<map::link>(std::forward<R>(rng), root, right)) and curr_height not_eq black_height)
						return false;
					if (detail::val<map::color>(std::forward<R>(rng), root, color) == map::color::red and detail::val<map::color>(std::forward<R>(rng), root, parent, color) == map::color::red)
						return false;
					...
					if (map::nil not_eq root and std::invoke(less, detail::ref(std::forward<R>(rng), root, key), detail::ref(std::forward<R>(rng), prev, key)))
						return false;
				}
				return true;
			}
		...
		struct lower_bound_fn
		{
			...
			auto operator()(R&& rng, map::link x, const V& value, Left left, Right right, Less less = {}, Key key = {}) const
			{
				auto z = map::nil;
				while (map::nil not_eq x)
		...
		struct find_fn
		{
			...
			auto operator()(R&& rng, map::link x, const V& value, Left left, Right right, Less less = {}, Key key = {}) const
			{
				...
				if (map::nil not_eq x and not std::invoke(less, value, detail::ref(std::forward<R>(rng), x, key)))
					return x;
				return map::nil;
			}
		};
